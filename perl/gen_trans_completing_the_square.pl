...
# ヘッダ プリアンブル
my $header =<<'HEADER';
\documentclass[landscape,b4paper,10pt]{ltjsarticle}

\usepackage[top=20truemm,bottom=10truemm,left=10truemm,right=10truemm]{geometry}

\usepackage{fancyhdr}
\pagestyle{fancy}
\fancyhead[L]{練習問題:平方完成 \hspace{50pt} 生成日:\today}
\fancyhead[R]{クラス\hspace{100pt}名前\hspace{200pt} /50}
\fancyfoot{}
...
#    print '\item $', $ques[$i], '$', "\n";
    ...
    if ($i!= $#ques and $i % 10 == 9){print '\columnbreak', "\n"}
}
