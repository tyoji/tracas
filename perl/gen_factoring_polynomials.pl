...
# メイン 問題解答生成
for (1..30) {
    my @number = (coprime_num(2, $num_range, 0), coprime_num(2, $num_range, 0));
    ...
    my $anser = '\left('
                       . trans_num($number[0],1) . 'x' . trans_num($number[1],2)
                       . '\right)';
    if ($number[0]==$number[2] && $number[1]==$number[3]) {
        $anser .= '^{2}';
    } else {
        $anser .= '\left(' . trans_num($number[2],1) . 'x' . trans_num($number[3],2) . '\right)';
    }
...
\usepackage{fancyhdr}
\pagestyle{fancy}
\fancyhead[L]{\fbox{練習問題} 因数分解 \qquad 生成日:\today}
